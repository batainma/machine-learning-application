...
regressor.add(LSTM(units = 24,activation='relu', return_sequences = True, input_shape = (X_train.shape[1], 1)))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
regressor.add(LSTM(units = 24, activation='relu'))
#regressor.add(Dropout(0.2))
regressor.add(Dense(units = 1))

regressor.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics=['accuracy'])

regressor.fit(X_train, y_train, epochs=1000, batch_size = 400)
...
plt.title('LSTM Predictions for Cell 000111 - 1000 epochs / relu')
