...
regressor.add(LSTM(units = 12, activation='sigmoid', return_sequences = True, input_shape = (X_train.shape[1], 1)))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
regressor.add(LSTM(units = 12, activation='sigmoid'))
#regressor.add(Dropout(0.2))
regressor.add(Dense(units = 1))

regressor.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics=['accuracy'])

regressor.fit(X_train, y_train, epochs=25, batch_size = 400)

test,predict = test[:-12],test[-12:]

data_test = dataset_scaled[len(dataset_scaled) - len(test) - 12 :]

print(data_test)

X_test = []
y_test = []
for i in range(12, len(data_test)):
    X_test.append(data_test[i-12:i,:])
...
plt.title('LSTM Predictions - 25 epochs / sigmoid / 12 hour')
