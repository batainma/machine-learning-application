...
X_train = []
y_train = []
for i in range(36, train.size):
    X_train.append(train[i-36:i, 0])
...
regressor.add(LSTM(units = 36, activation='sigmoid', return_sequences = True, input_shape = (X_train.shape[1], 1)))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
#regressor.add(LSTM(units = 24, return_sequences = True))
#regressor.add(Dropout(0.2))
regressor.add(LSTM(units = 36, activation='sigmoid'))
#regressor.add(Dropout(0.2))
regressor.add(Dense(units = 1))

regressor.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics=['accuracy'])

regressor.fit(X_train, y_train, epochs=100, batch_size = 500)

test,predict = test[:-36],test[-36:]

data_test = dataset_scaled[len(dataset_scaled) - len(test) - 36 :]

print(data_test)

X_test = []
y_test = []
for i in range(36, len(data_test)):
    X_test.append(data_test[i-36:i,:])
...
plt.title('LSTM Predictions - 100 epochs / sigmoid / 36 hour')
